...
{
	if (getMoviePath().empty()) return;  // No SWF for this scene
	loadMovie();
	mapElementsAndNames();
	updateSafeZone();
...
void UIScene::loadMovie()
{
	...
	if (!app.hasArchiveFile(moviePath))
	{
		...
		// Print the full directory and the relative path being checked
		app.DebugPrintf("WARNING: Could not find iggy movie at [%ls\\%ls], falling back on 720\n", cwd, moviePath.c_str());
		...
		if (!app.hasArchiveFile(moviePath))
		{
			app.DebugPrintf("ERROR: Could not find any iggy movie at [%ls\\%ls]!\n", cwd, moviePath.c_str());
...
void UIScene::gainFocus()
{
	if( !bHasFocus && stealsFocus() )
	{
		...
		*/
		...
		if (swf)  // ADD THIS guard around both
		{
			updateTooltips();
			updateComponents();
		}
